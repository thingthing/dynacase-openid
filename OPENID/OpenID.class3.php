...
class SimpleOpenID
{
	...
	private $arr_ax_types = array(
				...
				{
					error_log('url before='.$url);
					if (strchr($url, "index.php") !== FALSE) {
						$url= substr($url, 0, strpos($url, "index.php"));
					}
					if (strstr($url, "?")) {
						if (strchr($url, "&") !== FALSE && strchr($url, "&") === "&") {
							$url.= 'getOpenID' . $_POST["id"];
						}
						else {
							$url.= '&getOpenID' . $_POST["id"];
						}
				...
				public function redirect()
				{
					...
					//else {
					error_log('url a rediriger::' . $redirect_to);
				...
				public function validateWithServer()
				{
					...
					$endpoint_url = $this->getOpenIDServer();
				...
				private function makeCURLRequest($url, $method="GET", $params = "")
				{
					...
					if (curl_errno($curl) == 0) {
						...
						$redir_url = $class->parms{'authurl'} . '&openid_mode=notvalid';
